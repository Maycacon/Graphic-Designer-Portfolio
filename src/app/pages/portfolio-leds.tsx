...
import { ledsProjects as fallbackLeds } from "@/app/lib/fallback-projects";

interface PortfolioLedsProps {
  onNavigate: (page: string) => void;
}

export function PortfolioLeds({ onNavigate }: PortfolioLedsProps) {
  const { projects } = useAdminProjects();
  const [selectedProject, setSelectedProject] = useState<any | null>(null);
  const displayed = projects ? projects.filter((p: any) => p.category === 'Leds') : fallbackLeds.map((p: any) => ({ ...p, category: 'Leds' }));
  return (
